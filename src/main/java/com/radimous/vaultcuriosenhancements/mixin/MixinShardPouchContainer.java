...
@Mixin(value = ShardPouchContainer.class, remap = false)
public class MixinShardPouchContainer {
    ...
    private ItemStack getItem(Inventory instance, int i, Operation<ItemStack> original) {
        if (i == -1) {
            ItemStack stack = CuriosApi.getCuriosHelper().findFirstCurio(this.inventory.player, ModItems.SHARD_POUCH).map(SlotResult::stack).orElse(null);
            if (stack == null) {
